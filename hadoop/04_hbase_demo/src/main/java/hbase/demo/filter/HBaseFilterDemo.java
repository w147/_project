...
//		Filter filter = new RowFilter(CompareOp.EQUAL, new BinaryComparator(Bytes.toBytes("row8")));
...
//		Filter filter = new ValueFilter(CompareOp.EQUAL, new SubstringComparator("beijing"));
        ...
        Filter filter = new SingleColumnValueFilter(Bytes.toBytes("f1"), Bytes.toBytes("name"), CompareFilter.CompareOp.EQUAL, new SubstringComparator("xiaoming"));
